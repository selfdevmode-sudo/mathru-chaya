...
| F9 | Multilingual (EN/KN routes, strings files, fallback) | in progress | Task 2 done: i18n core module shipped — `src/lib/i18n.ts` with EN/KN strings, `t()` (English fallback), `pickText()`, `localePath()`, `otherLangs()`; 5 unit tests passing (`tests/i18n.test.ts`). EN/KN static routing (consuming this module in pages/layouts) lands in Tasks 5+, so F9 stays `in progress`. |
